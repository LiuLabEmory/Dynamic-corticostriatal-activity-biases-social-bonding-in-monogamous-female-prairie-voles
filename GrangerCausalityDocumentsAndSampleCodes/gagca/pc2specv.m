function [h, Granger, fs] = pc2specv(pc,R0,fn,T)
...
Granger=zeros(dim,dim,nspec);
fs = fs/T;
for t = 1:nspec,%number of freqs
	f = fs(t);
	Af = I;
	e = exp(-i*2*pi*T*f);
	for k = 1:order
   ...
   Afi = inv(Af);
	h(:,:,t) = T*Afi*Peps*Afi';
    for l=1:dim %number of channels
        for m=1:dim %number of channels
            Granger(l,m,t)=-log(1-((Peps(l,l)-(Peps(l,m)*Peps(l,m))/Peps(m,m))*abs(Afi(m,l))*abs(Afi(m,l)))/h(m,m,t));
        end
    end
end
